...
```{r imputeMissingValues, echo=FALSE, message=FALSE, warning=FALSE}
#set.seed(200)
...
#result1<-preProcess(beverage.train.clean, method = c("knnImpute"), k = 10)
#beverage.train.clean<-predict(result1, beverage.train.clean)
...
#result2<-preProcess(beverage.test.clean, method = c("knnImpute"), k = 10)
#beverage.test.clean<-predict(result2, beverage.test.clean)
...
beverage.test.clean <- mice(beverage.test.clean, m = 1, method = 'pmm', print = FALSE) %>% complete()
...
```{r}
#colnames(beverage.train.clean)

colnames(beverage.test.clean)
...
bev.test<-dummyVars(" ~ Brand.Code", data = beverage.test.clean)
#beverage.test.clean$PH <- 1
test_dummies<-predict(bev.test, beverage.test.clean)

# Now sort the eval_dummy columns so they match the training set dummies
test.dummy.cols <- sort(colnames(test_dummies))
test_dummies<-as.tibble(test_dummies)%>%dplyr::select(test.dummy.cols)

# Remove the original categorical variable
beverage.test.clean<-beverage.test.clean%>%dplyr::select(-c('Brand.Code'))

# Add the new dummy columns to the test dataframe
beverage.test.clean<-cbind(test_dummies, beverage.test.clean)
...
bev.test.predictors <- beverage.test.clean
...
preProcValues <- preProcess(bev.test.predictors, method = c("center", "scale", "BoxCox"))
bev.test.transformed <- predict(preProcValues, bev.test.predictors)
...
Now that we have identified the **_Cubist_** model as the best predictive model, we will apply the model to the evaluation dataset by replacing the empty **_PH** values in the set with the Cubist model's predictions.

```{r finalPHPreictions, eval=TRUE, message=FALSE, warning=FALSE}
# Clean evaluation data.
evaluationDataClean <- bev.test.transformed
...
evaluationDataClean$PH <- cubistPredictions

# Take a look at the evaluation data after PH value replacement.
evaluationDataClean %>% kable() %>% kable_styling() %>% scroll_box(width = '100%', height = '600px')
```
