...
ggplot(gather_df) + geom_histogram(aes(x=value, y = ..density..), bins=30)+geom_density(aes(x=value), color='red') + facet_wrap(. ~variable, scales='free', ncol=4)
```
...
# Test data splits.
#testDataSet <- bev.test.transformed[-trainingData, ]
TestData<-bev.test.transformed
#xTestData <- subset(testDataSet)
#yTestData <- subset(testDataSet)
```
...
svmPrediction <- predict(svmModel, newdata = TestData)
svmPerformance <- postResample(pred = svmPrediction, obs = TestData$PH)
svmPerformance
```
