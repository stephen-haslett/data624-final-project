...
# Load ABC Beverages train and evaluation datasets.
...
The results of running the training data through the _str()_ function reveal that the dataset consists of **33 _Variables_**, and **2571 _Observations_**. Almost all of the variables are numerical, with the exception of the **Brand.Code** variable. An other important revelation is that some of the variables contain missing values. 

In order to figure out how we should deal with these, we will now check to see how many missing variables exist.
...
beverage_df<-beverage.train%>%drop_na()%>%dplyr::select(-c(PH, 'Brand.Code'))%>%gather(key = 'variable', value = 'value')

# Plot histogram for each predictor
ggplot(beverage_df)+geom_histogram(aes(x=value, y = ..density..), bins=30)+geom_density(aes(x=value), color='red')+facet_wrap(. ~variable, scales='free', ncol=4)
...
beverage_df <- beverage.train %>% 
  dplyr::select(-c(PH, 'Brand.Code'))%>%tidyr::drop_na() %>%
...
# Use the kNN method from VIM package to impute missing values in our training data
beverage.train.clean<-beverage.train.clean %>% kNN(k=10) %>% dplyr::select(colnames(beverage.train.clean))

# Use the kNN method from VIM package to impute missing values in our evaluation data
...
The above results reveal that the dataset consists of **33 _Variables_**, and **267 _Observations_**. Again, some of the variables contain missing values, and so we will impute these using the **_mice()_** funtion and the **Predictive mean matching** method.
