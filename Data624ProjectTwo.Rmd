...
beverage.train.clean <- beverage.train.clean %>% 
  kNN(k = 10) %>% 
  dplyr::select(colnames(beverage.train.clean))
...
```{r cubistModel, echo=FALSE, message=FALSE, warning=FALSE}
set.seed(200)
...
```{r cubistModelPredict, echo=FALSE, message=FALSE, warning=FALSE}
cubistModelPrediction <- predict(cubistModel, newdata = xTestData)
postResample(pred = cubistModelPrediction, obs = yTestData$PH)
```


#### Single Tree Model

```{r singleTreeModel, echo=FALSE, message=FALSE, warning=FALSE}
set.seed(200)
...
```{r singleTreeModelPredict, echo=FALSE, message=FALSE, warning=FALSE}
singleTreeModelPrediction <- predict(singleTreeModel, newdata = xTestData)
postResample(pred = singleTreeModelPrediction , obs = yTestData$PH) 
```
